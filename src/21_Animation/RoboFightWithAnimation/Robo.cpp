...
mLockOn( true ){ //
	...
	mTree->setAnimation( mDatabase->animation( "standing" ) ); //站立动画
	...
	switch ( mMode ){
		...
		case MODE_WALKING:
			if ( iJump ){
				...
				mTree->setAnimation( mDatabase->animation( "jumping" ) ); //跳
			}else if ( iTurn ){
...
void Robo::move( bool left, bool right, bool up, bool down ){
	...
	if ( move.x != 0.0 || move.z != 0.0 ){
		if ( mMode == MODE_STANDING ){
			mMode = MODE_WALKING;
			mTree->setAnimation( mDatabase->animation( "walking" ) ); //开始步行
		}
	}else{
