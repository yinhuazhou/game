...
bool Pad::isOn( Button b, int id ) const {
	...
	if ( m.joystickNumber() > id ){
		...
		switch ( b ){
			case JUMP:
				r = ( j.isOn( 0 ) ) ? 1 : 0; //0号按钮跳跃
				break;
			case FIRE:
				r = ( j.isOn( 1 ) ) ? 1 : 0; //1号码键是武器
				break;
			case TURN:
				r = ( j.isOn( 2 ) ) ? 1 : 0; //2号码按钮旋转
				break;
			case UP:
...
bool Pad::isTriggered( Button b, int id ) const {
	...
	if ( m.joystickNumber() > id ){
		...
		switch ( b ){
			case JUMP:
				r = ( j.isTriggered( 0 ) ) ? 1 : 0; //0号按钮跳跃
				break;
			case FIRE:
				r = ( j.isTriggered( 1 ) ) ? 1 : 0; //1号码键是武器
				break;
			case TURN:
				r = ( j.isTriggered( 2 ) ) ? 1 : 0; //2号码按钮旋转
				break;
			case UP:
