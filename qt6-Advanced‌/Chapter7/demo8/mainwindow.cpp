...
{
    setGeometry(300,300,580,251);
    setWindowTitle("Q3DBars、QBar3DSeries、QBarDataProxy");
    //创建三维柱形图表
    graph3D=new Q3DBars();
    //创建三维图表容器
    ...
    //设置视角
    Q3DCamera::CameraPreset camView=Q3DCamera::CameraPresetLeftHigh;
    graph3D->scene()->activeCamera()->setCameraPreset(camView);
    //创建列标签、行标签
    QStringList colLabs={"One","Two","Three","Four","Five","Six","Seven"};
    QStringList rowLabs={"Week1","Week2","Week3","Week4"};
    //创建数据代理
    ...
    dataProxy->setColumnLabels(colLabs);        //添加列标签
    dataProxy->setRowLabels(rowLabs);           //添加行标签
