...
void MainWindow::listView_clicked(QModelIndex index){
    ...
    for(int i=0;i<header->count();i++){
        QVariant headerV=standardModel->horizontalHeaderItem(i)->data(Qt::DisplayRole);
        QString headerText=headerV.toString();
        if(headerText=="语文"||headerText=="数学"||headerText=="英文")
            tableView->setItemDelegateForColumn(i,delegate);//设置代理控件
    }
}
