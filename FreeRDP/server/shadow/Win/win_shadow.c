                                            ...
                                            UINT16 flags, UINT8 code)
{
	...
	event.DUMMYUNIONNAME.ki.wScan = code;
	event.DUMMYUNIONNAME.ki.dwFlags = KEYEVENTF_SCANCODE;
	event.DUMMYUNIONNAME.ki.dwExtraInfo = 0;
	event.DUMMYUNIONNAME.ki.time = 0;

	if (flags & KBD_FLAGS_RELEASE)
		event.DUMMYUNIONNAME.ki.dwFlags |= KEYEVENTF_KEYUP;

	if (flags & KBD_FLAGS_EXTENDED)
		event.DUMMYUNIONNAME.ki.dwFlags |= KEYEVENTF_EXTENDEDKEY;
                                                    ...
                                                    UINT16 code)
{
	...
	event.DUMMYUNIONNAME.ki.wVk = 0;
	event.DUMMYUNIONNAME.ki.wScan = code;
	event.DUMMYUNIONNAME.ki.dwFlags = KEYEVENTF_UNICODE;
	event.DUMMYUNIONNAME.ki.dwExtraInfo = 0;
	event.DUMMYUNIONNAME.ki.time = 0;
                                         ...
                                         UINT16 flags, UINT16 x, UINT16 y)
{
	...
	if (flags & (PTR_FLAGS_WHEEL | PTR_FLAGS_HWHEEL))
	{
		if (flags & PTR_FLAGS_WHEEL)
			event.DUMMYUNIONNAME.mi.dwFlags = MOUSEEVENTF_WHEEL;
		else
			event.DUMMYUNIONNAME.mi.dwFlags = MOUSEEVENTF_HWHEEL;
		event.DUMMYUNIONNAME.mi.mouseData = flags & WheelRotationMask;

		if (flags & PTR_FLAGS_WHEEL_NEGATIVE)
			event.DUMMYUNIONNAME.mi.mouseData *= -1;
	...
	else
	{
		...
		event.DUMMYUNIONNAME.mi.dx = (LONG)((float)x * (65535.0f / width));
		event.DUMMYUNIONNAME.mi.dy = (LONG)((float)y * (65535.0f / height));
		event.DUMMYUNIONNAME.mi.dwFlags = MOUSEEVENTF_ABSOLUTE;

		if (flags & PTR_FLAGS_MOVE)
		{
			event.DUMMYUNIONNAME.mi.dwFlags |= MOUSEEVENTF_MOVE;
			rc = SendInput(1, &event, sizeof(INPUT));
			if (rc == 0)
				return FALSE;
		}
		...
		if (flags & PTR_FLAGS_BUTTON1)
		{
			if (flags & PTR_FLAGS_DOWN)
				event.DUMMYUNIONNAME.mi.dwFlags |= MOUSEEVENTF_LEFTDOWN;
			else
				event.DUMMYUNIONNAME.mi.dwFlags |= MOUSEEVENTF_LEFTUP;
		...
		{
			if (flags & PTR_FLAGS_DOWN)
				event.DUMMYUNIONNAME.mi.dwFlags |= MOUSEEVENTF_RIGHTDOWN;
			else
				event.DUMMYUNIONNAME.mi.dwFlags |= MOUSEEVENTF_RIGHTUP;
		...
		{
			if (flags & PTR_FLAGS_DOWN)
				event.DUMMYUNIONNAME.mi.dwFlags |= MOUSEEVENTF_MIDDLEDOWN;
			else
				event.DUMMYUNIONNAME.mi.dwFlags |= MOUSEEVENTF_MIDDLEUP;
                                                  ...
                                                  UINT16 y)
{
	...
	if ((flags & PTR_XFLAGS_BUTTON1) || (flags & PTR_XFLAGS_BUTTON2))
	{
		...
		if (flags & PTR_FLAGS_MOVE)
		{
			...
			event.DUMMYUNIONNAME.mi.dx = (LONG)((float)x * (65535.0f / width));
			event.DUMMYUNIONNAME.mi.dy = (LONG)((float)y * (65535.0f / height));
			event.DUMMYUNIONNAME.mi.dwFlags = MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE;
			rc = SendInput(1, &event, sizeof(INPUT));
			if (rc == 0)
				return FALSE;
		}

		event.DUMMYUNIONNAME.mi.dx = event.DUMMYUNIONNAME.mi.dy = event.DUMMYUNIONNAME.mi.dwFlags = 0;

		if (flags & PTR_XFLAGS_DOWN)
			event.DUMMYUNIONNAME.mi.dwFlags |= MOUSEEVENTF_XDOWN;
		else
			event.DUMMYUNIONNAME.mi.dwFlags |= MOUSEEVENTF_XUP;

		if (flags & PTR_XFLAGS_BUTTON1)
			event.DUMMYUNIONNAME.mi.mouseData = XBUTTON1;
		else if (flags & PTR_XFLAGS_BUTTON2)
			event.DUMMYUNIONNAME.mi.mouseData = XBUTTON2;
